...
class PDFToExcelUploadParams(BaseModel):
    files: List[UploadFile]


def pdf_to_excel_upload_params(
    files: List[UploadFile] = File(...),
) -> PDFToExcelUploadParams:
    return PDFToExcelUploadParams(files=files)


class PDFToExcelProcessParams(BaseModel):
    session_id: str
    pages: str = "all"
...
async def upload_pdf_for_excel(
    ...
    params: PDFToExcelUploadParams = Depends(pdf_to_excel_upload_params),
):
    files = params.files
    if len(files) < 1:
...
async def process_pdf_to_excel(
    background_tasks: BackgroundTasks,
    params: PDFToExcelProcessParams = Depends(),
):
    ...
    try:
        ...
        try:
            ...
            if page_analysis["has_tables"] and page_analysis["has_paragraphs"]:
                ...
                tables = _process_hybrid_content(
                    pdf_file,
                    pages,
                    password,
                    detection_method,
                ...
                tables = _extract_tables_only(
                    pdf_file, pages, password, detection_method
                )
            else:
                # Sadece paragraf var - metin olarak çıkar
                tables = _extract_paragraphs_only(
                    pdf_file, pages, password, merge_paragraphs, min_paragraph_lines
                )
            ...
            try:
                ...
                try:
                    if detection_method == "lattice":
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Lattice metodu başarısız, stream metodunu deniyorum: {str(lattice_error)}"
                            )
                ...
                raise HTTPException(
                    status_code=400,
                    detail="PDF'den veri çıkarılamadı. Dosya şifreli olabilir veya işlenemiyor.",
                )

        if not tables or len(tables) == 0:
            raise HTTPException(
                status_code=400, detail="PDF dosyasında tablo bulunamadı"
            )
        ...
        raise HTTPException(
            status_code=500, detail="PDF'den Excel dönüştürme sırasında hata oluştu"
        )
...
def _extract_text_with_pymupdf(
    pdf_file: str, pages: str, password: str = None
) -> List[pd.DataFrame]:
                ...
                if "lines" in block:  # Metin bloğu
                    for line in block["lines"]:
                        ...
                        if line_text.strip():
                            ...
                            if (
                                line_text.strip() == "" or len(current_row) > 5
                            ):  # Maksimum 5 hücre
                                if current_row:
...
def _merge_paragraphs_in_tables(
    tables: List[pd.DataFrame], min_lines: int = 5
) -> List[pd.DataFrame]:
...
def _analyze_pdf_content(pdf_file: str, pages: str, password: str = None) -> dict:
    ...
    try:
        ...
        if table_indicators > 0 and total_text_length > 0:
            table_ratio = table_indicators / (
                total_text_length / 100
            )  # 100 karakter başına tablo göstergesi
            has_tables = (
                table_ratio > 0.1
            )  # %10'dan fazla tablo göstergesi varsa tablo var

        # Paragraf varlığını belirle
        if (
            not has_paragraphs and total_text_length > 500
        ):  # Uzun metin varsa paragraf var
            has_paragraphs = True
...
def _extract_tables_only(
    pdf_file: str, pages: str, password: str, detection_method: str
) -> List[pd.DataFrame]:
...
def _process_hybrid_content(
    ...
    try:
        ...
        paragraphs = _extract_paragraphs_only(
            pdf_file, pages, password, merge_paragraphs, min_paragraph_lines
        )
